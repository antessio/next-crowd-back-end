...
import { getEntities as getProjectOwners } from 'app/entities/project-owner/project-owner.reducer';
import { Status } from 'app/shared/model/enumerations/status.model';
import { createEntity, getEntity, reset, updateEntity } from './crowdfunding-project.reducer';
...
export const CrowdfundingProjectUpdate = () => {
  ...
  const projectOwners = useAppSelector(state => state.projectOwner.entities);
  ...
  const handleClose = () => {
    navigate(`/crowdfunding-project${location.search}`);
  };
  ...
  const saveEntity = values => {
    ...
    const entity = {
      ...
      ...values,
      owner: projectOwners.find(it => it.id.toString() === values.owner?.toString()),
    };
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              />
              <ValidatedField
                label={translate('nextCrowdBackEndApp.crowdfundingProject.status')}
                id="crowdfunding-project-status"
                name="status"
                data-cy="status"
                type="select"
              >
                {statusValues.map(status => (
                  <option value={status} key={status}>
                    {translate(`nextCrowdBackEndApp.Status.${status}`)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
              ...
              />
              <ValidatedField
                label={translate('nextCrowdBackEndApp.crowdfundingProject.longDescription')}
                id="crowdfunding-project-longDescription"
